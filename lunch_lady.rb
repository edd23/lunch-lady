...
class Side < Food
  ...
  @nutricion = {
    ...
    lasagna: {fat: 40, calories: 345, carbs: 40},
    fries: {fat: 15, calories: 320, carbs: 43},
    salad: {fat: 10, calories: 100, cabs: 18},
    soup: {fat: 14, calories: 120, carbs: 18},
    brownie: {fat:25, calories: 420, carbs: 46},
    cookie: {fat: 15, calories: 220, carbs: 30}
  }
  ...
  # I commented all this part, becuase i couldn't make it to work and when I had it it was messing up other things. But everything else works without it.
  #def main_nutrition 
   # puts "#{@nutricion[:pizza][:fat23][:calories500][:carbs45]}"
    #puts "#{@nutricion[:tacos][fat:18][calories: 360][carbs: 34]}"
    #puts "#{@nutricion[:burger][fat: 33][calories: 590][carbs: 43]}"
    #puts "#{@nutricion[:lasagna][fat: 30][calories: 344][carbs: 39]}"
    #puts "#{@nutricion[:fries][fat: 15][calories: 320][carbs: 43]}"
    #puts "#{@nutricion[:salad][fat: 13][calories: 210][carbs: 18]}"
    #puts "#{@nutricion[:soup][fat: 5][calories: 110][carbs: 17]}"
    #puts "#{@nutricion[:brownie][fat: 24][calories: 410][carbs: 46]}"
    #puts "#{@nutricion[:cookie][fat: 10][calories: 220][carbs: 30]}"
...
def check_out
  ...
  def farewell
    ...
    puts "Thank you for visiting the Garcia's Lunch Room! Have a nice day!"
    exit(0)
  ...
  while true
    @choices.clear
    @sum.clear
    puts "Welcome to the Garcia's Lunch Room!"
